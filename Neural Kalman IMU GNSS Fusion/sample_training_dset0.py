...
# 모델 학습
history = model.fit(x=X_train, y=[x_vel_train, y_vel_train], epochs=5000, shuffle=True,
...
# Find the minimum length to avoid any out-of-bound errors
min_len = min(len(Gvx), len(Gvy), len(Pvx), len(Pvy))

# Plot the results
plt.figure()
plt.plot(Gvx[0:min_len], Gvy[0:min_len], label='Ground Truth')
plt.plot(Pvx[0:min_len], Pvy[0:min_len], label='Predicted')
plt.legend()
plt.xlabel('X')
plt.ylabel('Y')
...
# Print the trajectory length
print('Trajectory Length:', Cal_len_meters(Gvx[0:min_len], Gvy[0:min_len]))
